...
var McpClient = class {
  ...
  async start() {
    ...
    const normEnv = this.env ? Object.fromEntries(Object.entries(this.env).filter(([_, v]) => typeof v === "string")) : void 0;
...
var TableItem = class extends vscode.TreeItem {
  constructor(label) {
...
async function activate(context) {
  ...
  const startCmd = vscode.commands.registerCommand("mssqlAssistant.start", async () => {
    ...
    const serverDir = import_path.default.join(context.extensionPath, "..", "MssqlMcpServer", "Node");
    const serverEntry = import_path.default.join(serverDir, "dist", "index.js");
  ...
  const runToolCmd = vscode.commands.registerCommand("mssqlAssistant.runTool", async () => {
    ...
    vscode.workspace.openTextDocument({ content: JSON.stringify(result, null, 2), language: "json" }).then((doc) => vscode.window.showTextDocument(doc, { preview: false }));
  });
...
async function tryPopulateTables(client2, provider, showErrors = false) {
  try {
    await client2.refreshTools();
    ...
    const result = await client2.callTool(candidate, {});
...
function extractTableNames(result) {
    ...
    for (const c of result.content) {
      if (c?.type === "text" && typeof c.text === "string") {
        try {
          ...
          if (Array.isArray(parsed?.tables)) return parsed.tables.filter((t) => typeof t === "string");
        } catch {
        }
      }
    }
  ...
  if (Array.isArray(possible?.tables)) return possible.tables.filter((t) => typeof t === "string");
  return [];
}
